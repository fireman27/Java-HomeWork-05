...
public class Task02 {
    ...
    public static void nameRepeat(Map<String, Integer> map){
        for(var item: map.entrySet()){
            if (item.getValue() > 1) System.out.printf("%s - %d; ", item.getKey(), item.getValue());
        }
    }
    public static void sortName(Map<String, Integer> map){
        ...
        for (int i = listCount.size()-1; i > -1; i--){
            for (var item: map.entrySet()){
                if (listCount.get(i) == item.getValue()) System.out.printf("%s - %d; ", item.getKey(), item.getValue());
            }
        }
    }
    public static void main(String[] args) {
        ...
        nameRepeat(mapName);
        System.out.println("\nИмена, отсортированные по убыванию и популярности: ");
        sortName(mapName);
    }
}
